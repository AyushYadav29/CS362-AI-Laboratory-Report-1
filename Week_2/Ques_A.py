...
def get_successors(state):
    ...
    for cell in range(0, len(state.board)):
        if state.board[cell] == 1:
            moveNorth = move_north(state, cell)
            moveSouth = move_south(state, cell)
            moveWest = move_west(state, cell)
            moveEast = move_east(state, cell)
            moves.append(moveNorth)
            moves.append(moveSouth)
            moves.append(moveWest)
            moves.append(moveEast)
            if moveNorth is None and moveSouth is None and moveEast is None and moveWest is None:
                isolated += 1
    for move in moves:
        if move:
...
def aStar(problem, heuristic):
    hq, origin, cost = MyPriorityQueue(), {}, {}
    hq.put(problem, 0)
...
def astar_h1_solution():
    print('[===============================A* Heuristic1-Number of successor nodes===============================]')
    sTime = time.time()
...
def astar_h2_solution():
    print('[===============================A* Heuristic2-Number of isolated peg\'s===============================]')
    sTime = time.time()
    game = State(bfs_board, dfs_goal, 7, 7, count_pegs(dfs_board))
    # print(game)
...
def astar_h3_solution():
    print('[===============================A* Heuristic3- average of H1 and H2===============================]')
    sTime = time.time()
    game = State(bfs_board, dfs_goal, 7, 7, count_pegs(dfs_board))
    # print(game)
